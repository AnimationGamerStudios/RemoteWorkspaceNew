...
package net.mcreator.life_statues.world.dimension;
...
import net.mcreator.life_statues.item.TheStatuesDimensionItem;
import net.mcreator.life_statues.block.UnknownGreenWoodStairsBlock;
...
public class TheStatuesDimensionDimension extends LifeStatuesModElements.ModElement {
	@ObjectHolder("life_statues:the_statues_dimension_portal")
	public static final CustomPortalBlock portal = null;

	public TheStatuesDimensionDimension(LifeStatuesModElements instance) {
		super(instance, 47);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(Blocks.DARK_OAK_PLANKS);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("life_statues:the_statues_dimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("the_statues_dimension_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("the_statues_dimension_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("the_statues_dimension_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new TheStatuesDimensionItem().setRegistryName("the_statues_dimension"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("life_statues:the_statues_dimension"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("life_statues:the_statues_dimension")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == UnknownGreenWoodStairsBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? UnknownGreenWoodStairsBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, UnknownGreenWoodStairsBlock.block.getDefaultState(), 3);
					}
				}
			}
